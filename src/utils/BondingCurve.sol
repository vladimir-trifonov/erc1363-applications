 ...
 * of Ether being spent, and MULTIPLIER is 10^6.
 *
 * The `calculatePriceForTokens` function takes an amount of tokens and the current token supply  
 * as input and returns the cost of purchasing that amount of tokens in Ether.
 *
 * The `calculateTokensForPrice` function takes an amount of Ether and the current token supply  
 * as input and returns the number of tokens that can be purchased with that amount of Ether.
 *
 * The `cubeRoot` function calculates the cube root of a non-negative integer using the nthRoot  
 * function implemented using the binary search algorithm.
 *
 * The `nthRoot` function calculates the integer n-th root of a non-negative integer using the  
 * binary search algorithm. The function has a time complexity of O(log x) and can only calculate  
 * the integer n-th root of a non-negative integer x.
 */
library BondingCurve {
    ...
    function calculateTokensForPrice(
        ...
        return
            cubeRoot((amount / MULTIPLIER) * 3 + (supply + 1) ** 3) -
            supply -
            1;
    }
    ...
    // The nthRoot function calculates the integer n-th root of a non-negative integer x
